...
app (file out, file err) run_gc (file shfile, string param_line, string instance)
{
...
//run_prerequisites() => {
  file model_sh = input(emews_root+"/scripts/GC.sh");
  ...
  foreach s,i in upf_lines {
    ...
    make_dir(instance) => {
      ...
      (out,err) = run_gc(model_sh, s, instance);
    }
  }
//}
